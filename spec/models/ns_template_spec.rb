...
describe NsTemplate do
  ...
  context "validation" do
    [:sc_event_id, :template_code, :display_text].each do |att|
      it { should validate_presence_of(att) }
    end

    it do
      ns_template = Factory(:ns_template, approval_status: 'A')
      should validate_uniqueness_of(:template_code).scoped_to(:approval_status)
    end
    
    it { should validate_length_of(:display_text).is_at_most(100) }
  ...
  context 'get_template_code' do
    it 'should return the reference_no for NsTemplate record' do
      ns_template1 = Factory(:ns_template, :approval_status => 'A') 
      ns_template2 = Factory(:ns_template, :approval_status => 'A') 
      ns_template3 = Factory.build(:ns_template, :approval_status => 'U')
      ns_template3.get_template_code.should == 'TEM3'
    end
  end
end
